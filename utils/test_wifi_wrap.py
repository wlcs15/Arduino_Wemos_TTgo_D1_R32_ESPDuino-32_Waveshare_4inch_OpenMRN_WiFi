...
def main():
    run([PY, "-u", os.path.join(ROOT, "utils", "wifi_wrap.py"), "selftest"])

    testdata = os.path.join(ROOT, "utils", "testdata", "fake_debug_ids.txt")
    ...
    run(
        [
            PY,
            "-u",
            os.path.join(ROOT, "utils", "collect_hw_ids.py"),
            "--from-log",
            testdata,
            "--out",
            ids_path,
    ...
    if "WIFI_NODE_ID=05.01.01.01.A5.01" not in text:
    ...
    proc = subprocess.Popen(
        [
            PY,
            "-u",
            os.path.join(ROOT, "utils", "wifi_wrap.py"),
            "encrypt",
            "--ids",
            ids_path,
            "--ssid",
    ...
    if "kWifiWrapSsid" not in wrap:
        raise SystemExit("wrap SSID missing")

    print("== provision_wifi_build.py refuses non-TTY ==")
    env = os.environ.copy()
    env.pop("WIFI_PASSWORD", None)
    proc = subprocess.Popen(
        [PY, "-u", os.path.join(ROOT, "utils", "provision_wifi_build.py")],
        stdin=subprocess.PIPE,
